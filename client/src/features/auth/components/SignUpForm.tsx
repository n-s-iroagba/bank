...
const{superAdminData,errorMessage,handleChange,showPassword,passwordValidityMessage,passwordType,handleSubmit,setSuperAdminData,handleChangeForConfirmPassword,isMatchingPassword} = useContext(AuthContext)
  ...
  <div className='form-wrapper'>
    ...
    <Form  className="mx-auto w-100 w-lg-25"  onSubmit={(e)=>handleSubmit(e,navigate)}>
    <Form.Group>
...
<br/>
      <Form.Group >
        ...
        />
              ...
              <FontAwesomeIcon className='px-3' onClick={() => showPassword()} icon={passwordType === 'text' ? faEye : faEyeSlash}/>
          </div>
...
<br/>
          {superAdminData.confirmPassword && !isMatchingPassword && <p className=' mb-0 text-danger'>***passwords do not match.</p>}
      </Form.Group>
      <br/>
      <Form.Group >
        ...
        <p className='fs-6 text-center'>***The code provided to you by the developer.</p>
      </Form.Group>
     
      <Button className='w-100 button-radius bg-blue' type="submit">Sign Up</Button>

    </Form>

    <br/>
    {errorMessage && <Alert variant='danger' className='min-width-100 text-center'>{errorMessage}</Alert>}
    </div>
  );
};
